...
export default function App() {
  ...
  useEffect(() => { localStorage.setItem('mynav_links', JSON.stringify(links)); }, [links]);
  useEffect(() => { localStorage.setItem('mynav_categories', JSON.stringify(categories)); }, [categories]);
  useEffect(() => {
  ...
  const handleAiQuery = async () => {
    ...
    setIsThinking(true); setAiResponse(null);
    const res = await askGemini(searchQuery);
    setAiResponse(res); setIsThinking(false);
  };
  ...
  const bgStyle = settings.backgroundImageUrl ? { backgroundImage: `url(${settings.backgroundImageUrl})`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed' } : {};

  if (isLocked) return <LockScreen onUnlock={(pwd) => { if (pwd === settings.password) { setIsLocked(false); return true; } return false; }} />;

  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0f172a] text-slate-800 dark:text-slate-200 pb-20 transition-colors duration-500" style={bgStyle}>
      {!settings.backgroundImageUrl && <div className="fixed inset-0 -z-10 bg-gradient-to-br from-slate-50 to-slate-200 dark:from-[#1e293b] dark:to-[#0f172a]" />}
      <div className="absolute top-4 right-4 flex gap-3 z-20">
        <button onClick={() => setIsHelpOpen(true)} className="p-2 rounded-full hover:bg-slate-200/50 dark:hover:bg-white/10 bg-white/50 dark:bg-black/20 backdrop-blur-sm"><HelpCircle size={20} /></button>
        <button onClick={() => setIsImageEditorOpen(true)} className="p-2 rounded-full hover:bg-slate-200/50 dark:hover:bg-white/10 bg-white/50 dark:bg-black/20 backdrop-blur-sm"><Wand2 size={20} /></button>
        <button onClick={() => setIsSettingsOpen(true)} className="p-2 rounded-full hover:bg-slate-200/50 dark:hover:bg-white/10 bg-white/50 dark:bg-black/20 backdrop-blur-sm"><SettingsIcon size={20} /></button>
      </div>
      <div className="pt-20 pb-8 px-4 flex flex-col items-center text-center">
        <div className="mb-8"><h1 className="text-6xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-cyan-600 to-blue-700 dark:from-cyan-400 dark:to-blue-600">{time.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</h1><p className="mt-3 text-lg">{time.toLocaleDateString()}</p></div>
